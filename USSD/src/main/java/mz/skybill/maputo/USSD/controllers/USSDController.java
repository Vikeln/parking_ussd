package mz.skybill.maputo.USSD.controllers;
...
import mz.skybill.matola.ussd.repos.SessionRepository;
import mz.skybill.matola.ussd.services.ManipulationService;
import mz.skybill.matola.ussd.services.MatolaService;
import mz.skybill.matola.ussd.services.USSDService;
import mz.skybill.matola.ussd.utils.*;
...
public class USSDController {
    ...
    private String processRequest(HttpServletResponse response, String msisdn, String input, String sessionId, String operator, String serviceCode) {
        ...
        if (!isNew) {
            ...
            if (text.startsWith("END") || previousLog.getScreenText().contains("Enter Last Name")){
                ...
                log.info("MENU_INPUTS: {}",session.getSessionInputs());
    ...
    public String mainMenu(Session session, Object object) {
        ...
        if (session.isRegistered() && session.getAccessToken() != null){
            switch (session.getSessionInputs().stream().findFirst().get()) {
                case "0":
                    ...
                    if(dbSession != null){
                        ...
                        text = USSDMenu.getMatolaUssdMessage(session,inputs,object);
                    }
                    break;
            ...
            text = USSDMenu.registerUser(session,inputs);
